...
export default function Wpc2gTchBlock({ title, level }: { title: string; level: string }) {
  const { dateRange2, filter, siteId, nop, kabupaten, batch } = useFilterStore();

  const { isPending, error, data, isFetching, refetch } = useQuery({
    queryKey: ["2g-tch-block", dateRange2, filter, siteId, nop, kabupaten, batch],
    queryFn: async () => {
      const response = await fetch(
        `/api/gefr/wpc/2g-tch-block/?batch=${batch}&nop=${level === "NOP" ? nop : kabupaten}&kabupaten=${level === "NOP" ? nop : kabupaten}&tgl_1=${dateRange2?.split("|")[0]}&tgl_2=${dateRange2?.split("|")[1]}`,
      );
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-left">TCH Block Fail Number</TableHead>
              ...
              <TableBody>
                {data.rows.map((row: any, index: any) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="">{row["TCH_BLOCK_FAIL_NUM"]}</TableCell>
                    <TableCell className="">{row["TCH_BLOCK_SR"].toFixed(2)}</TableCell>
                    <TableCell className="">{row["SD_BLOCK_SR"].toFixed(2)}</TableCell>
