...
export default function Wpc2gSdBlock({ title, level }: { title: string; level: string }) {
  const { dateRange2, filter, siteId, nop, kabupaten, batch } = useFilterStore();

  const { isPending, error, data, isFetching } = useQuery({
    queryKey: ["2g-sd-block", dateRange2, filter, siteId, nop, kabupaten, batch],
    queryFn: async () => {
      const response = await fetch(
        `/api/gefr/wpc/2g-sd-block/?batch=${batch}&nop=${level === "NOP" ? nop : kabupaten}&kabupaten=${level === "NOP" ? nop : kabupaten}&tgl_1=${dateRange2?.split("|")[0]}&tgl_2=${dateRange2?.split("|")[1]}`,
      );
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-left">SD Block Fail Number</TableHead>
              ...
              <TableBody>
                {data.rows.map((row: any, _index: any) => (
                  <TableRow key={row.NOP}>
                    ...
                    <TableCell className="">{row.SD_BLOCK_FAIL_NUM}</TableCell>
                    <TableCell className="">{row.SD_BLOCK_SR.toFixed(2)}</TableCell>
                    <TableCell className="">{row.TCH_AVAIL_SR.toFixed(2)}</TableCell>
