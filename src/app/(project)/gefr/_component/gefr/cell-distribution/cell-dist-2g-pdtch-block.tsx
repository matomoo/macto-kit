...
import CellDistributionChartByPercentage from "./chart-cell-distribution-by-percentage";
...
export default function CellDist2gPdtchBlock({ title, level }: { title: string; level: string }) {
  ...
  const { isPending, error, data, isFetching } = useQuery({
    queryKey: ["cell-distribution-2g-pdtch-block", dateRange2, filter, siteId, nop, kabupaten],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/gefr/cell-distribution/2g-pdtch-block/?${params.toString()}`);
      ...
      {isFetching ? (
        ...
        <div className="grid min-h-0 grid-cols-1 gap-4">
          <div className="rounded-lg border bg-white p-4 shadow-sm">
            <CellDistributionChartByPercentage data={processedData} title={title} />
