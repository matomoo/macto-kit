...
const CellDistributionChart: React.FC<CellDistributionChartProps> = ({ data, title }) => {
  // Calculate percentage data for the chart
  const percentageData = data.map((item) => {
    const total = parseInt(item.total_records, 10);
    return {
      "<2%": (parseInt(item["<2%"], 10) / total) * 100,
      "2-5%": (parseInt(item["2-5%"], 10) / total) * 100,
      ">5%": (parseInt(item[">5%"], 10) / total) * 100,
    };
  });

  const cellCountData = data.map((item) => ({
    "<2%": parseInt(item["<2%"], 10),
    "2-5%": parseInt(item["2-5%"], 10),
    ">5%": parseInt(item[">5%"], 10),
  }));

  const calculateYAxisMin = () => {
    if (data.length === 0) return 95;

    const minPercentage = Math.min(...percentageData.map((item) => item["<2%"] + item["2-5%"] + item[">5%"]));

    // console.log(minPercentage)

    // Start from 95% or the minimum percentage in your data, whichever is lower
    return Math.min(85, Math.floor(minPercentage));
  };
    ...
    plugins: {
        ...
        callbacks: {
          label: (context: TooltipItem<"bar">) => {
            ...
            const actualCount = parseInt(
              item[context.dataset.label === "<2%" ? "<2%" : context.dataset.label === "2-5%" ? "2-5%" : ">5%"],
              10,
            );
          ...
          afterBody: (context: TooltipItem<"bar">[]) => {
            ...
            return [
              ...
              `<2%: ${item["<2%"]} (${((parseInt(item["<2%"], 10) / parseInt(item.total_records, 10)) * 100).toFixed(1)}%)`,
              `2-5%: ${item["2-5%"]} (${((parseInt(item["2-5%"], 10) / parseInt(item.total_records, 10)) * 100).toFixed(1)}%)`,
              `>5%: ${item[">5%"]} (${((parseInt(item[">5%"], 10) / parseInt(item.total_records, 10)) * 100).toFixed(1)}%)`,
            ];
          },
        },
      },
      ...
      datalabels: {
        ...
        formatter: (_value: number, context: any) => {
          ...
          let cellCount = 0;
          if (datasetIndex === 0) {
            cellCount = cellCountData[dataIndex]["<2%"];
          } else if (datasetIndex === 1) {
            cellCount = cellCountData[dataIndex]["2-5%"];
          } else if (datasetIndex === 2) {
            cellCount = cellCountData[dataIndex][">5%"];
          }
  ...
  const chartData: ChartData<"bar"> = {
    ...
    datasets: [
      {
        label: "<2%",
        data: percentageData.map((item) => item["<2%"]),
      ...
      {
        label: "2-5%",
        data: percentageData.map((item) => item["2-5%"]),
      ...
      {
        label: ">5%",
        data: percentageData.map((item) => item[">5%"]),
