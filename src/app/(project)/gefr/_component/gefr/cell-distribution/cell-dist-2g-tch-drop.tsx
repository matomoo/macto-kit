...
export default function CellDist2gTchDrop({ title, level }: { title: string; level: string }) {
  ...
  const { isPending, error, data, isFetching, refetch } = useQuery({
    queryKey: ["cell-distribution-2g-tch-drop", dateRange2, filter, siteId, nop, kabupaten],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/gefr/cell-distribution/2g-tch-drop/?${params.toString()}`);
