...
export default function CellDist2gTchBlock({ title, level }: { title: string; level: string }) {
  ...
  const { isPending, error, data, isFetching } = useQuery({
    queryKey: ["cell-distribution-2g-tch-block", dateRange2, filter, siteId, nop, kabupaten],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/gefr/cell-distribution/2g-tch-block/?${params.toString()}`);
