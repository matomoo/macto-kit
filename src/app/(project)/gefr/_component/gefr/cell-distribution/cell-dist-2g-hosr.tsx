...
import CellDistributionChartByPercentageHosr from "./chart-cell-distribution-by-percentage-hosr";
...
export default function CellDist2gHosr({ title, level }: { title: string; level: string }) {
  ...
  const { isPending, error, data, isFetching } = useQuery({
    queryKey: ["cell-distribution-2g-hosr", dateRange2, filter, siteId, nop, kabupaten],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/gefr/cell-distribution/2g-hosr/?${params.toString()}`);
      ...
      {isFetching ? (
        ...
        <div className="grid min-h-0 grid-cols-1 gap-4">
          <div className="rounded-lg border bg-white p-4 shadow-sm">
            <CellDistributionChartByPercentageHosr data={processedData} title={title} />
