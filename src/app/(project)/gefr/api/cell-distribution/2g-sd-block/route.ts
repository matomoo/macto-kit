  ...
  try {
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg4gModel>(sql`
      SELECT
        t1."Begin Time",
        COUNT(CASE 
                WHEN (t1."SDCCH_Block_num_ono" = 0 AND t1."SDCCH_Block_denum_ono" = 0) THEN 1
                WHEN (t1."SDCCH_Block_num_ono" :: DECIMAL / NULLIF (t1."SDCCH_Block_denum_ono", 0) * 100) <= 2 THEN 1 
              END) AS "<2%",
        COUNT(CASE WHEN (t1."SDCCH_Block_num_ono" :: DECIMAL / NULLIF (t1."SDCCH_Block_denum_ono", 0) * 100) BETWEEN 2 AND 5 THEN 1 END) AS "2-5%",
        COUNT(CASE WHEN (t1."SDCCH_Block_num_ono" :: DECIMAL / NULLIF (t1."SDCCH_Block_denum_ono", 0) * 100) > 5 THEN 1 END) AS ">5%",
