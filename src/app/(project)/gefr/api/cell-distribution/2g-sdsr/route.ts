  ...
  try {
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg4gModel>(sql`
      SELECT
        t1."Begin Time",
        COUNT(CASE 
                WHEN (t1."SDSR_num_ono" = 0 AND t1."SDSR_denum_ono" = 0) THEN 1
                WHEN (t1."SDSR_num_ono" :: DECIMAL / NULLIF (t1."SDSR_denum_ono", 0) * 100) < 98 THEN 1 
              END) AS "<98%",
        COUNT(CASE WHEN (t1."SDSR_num_ono" :: DECIMAL / NULLIF (t1."SDSR_denum_ono", 0) * 100) >= 98 THEN 1 END) AS ">98%",
