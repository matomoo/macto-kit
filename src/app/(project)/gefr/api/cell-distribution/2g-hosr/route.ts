  ...
  try {
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg4gModel>(sql`
      SELECT
        t1."Begin Time",
        COUNT(CASE 
                WHEN (t1."HOSR_num_ono" = 0 AND t1."HOSR_denum_ono" = 0) THEN 1
                WHEN (t1."HOSR_num_ono" :: DECIMAL / NULLIF (t1."HOSR_denum_ono", 0) * 100) <= 90 THEN 1 
              END) AS "<90%",
        COUNT(CASE WHEN (t1."HOSR_num_ono" :: DECIMAL / NULLIF (t1."HOSR_denum_ono", 0) * 100) BETWEEN 90 AND 96 THEN 1 END) AS "90-96%",
        COUNT(CASE WHEN (t1."HOSR_num_ono" :: DECIMAL / NULLIF (t1."HOSR_denum_ono", 0) * 100) > 96 THEN 1 END) AS ">96%",
