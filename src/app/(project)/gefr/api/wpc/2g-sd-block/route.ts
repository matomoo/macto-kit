...
export async function GET(request: Request) {
  ...
  try {
    ...
    // console.log("queryNOP", queryNOP);
    // console.log("queryKabupaten", queryKabupaten);
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg4gModel>(sql`
      SELECT
        ...
        t1."SITE ID",
        t1."BTS ID",
        (SUM(t1."SDCCH_Block_denum_ono") - SUM(t1."SDCCH_Block_num_ono")) AS "SD_BLOCK_FAIL_NUM",
      ...
      GROUP BY
        ...
        t1."BTS ID"
      HAVING
        (SUM(t1."SDCCH_Block_num_ono") / NULLIF(SUM(t1."SDCCH_Block_denum_ono"),0) * 100 ) > 1
      ORDER BY
        (SUM(t1."SDCCH_Block_denum_ono") - SUM(t1."SDCCH_Block_num_ono")) DESC
        LIMIT 20;
    `);
