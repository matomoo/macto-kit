...
export async function GET(request: Request) {
  ...
  try {
    ...
    // console.log(
    //   formattedTglAfter1.toISOString(),
    //   formattedTglAfter2.toISOString()
    // );
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg4gModel>(sql`
      SELECT
        ...
        t1."SITE ID",
        t1."BTS ID",
        (SUM(t1."HOSR_denum_ono") - (SUM(t1."HOSR_num_ono"))) AS "HO_NUM_FAIL",
        (SUM(t1."HOSR_num_ono") / NULLIF(SUM(t1."HOSR_denum_ono"),0) * 100 ) AS "HO_SR",
      ...
      GROUP BY
        ...
        t1."BTS ID"
      HAVING
        (SUM(t1."HOSR_num_ono") / NULLIF(SUM(t1."HOSR_denum_ono"),0) * 100 ) < 96 
      ORDER BY
        (SUM(t1."HOSR_denum_ono") - SUM(t1."HOSR_num_ono")) DESC
      LIMIT 40;
    `);
