  ...
  try {
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg4gModel>(sql`
      SELECT
        ...
        t1."SITE ID",
        t1."BTS ID",
        (SUM(t1."TCH_Drop_num_ono")) AS "TCH_DROP_FAIL_NUM",
        (SUM(t1."TCH_Drop_num_ono") / NULLIF(SUM(t1."TCH_Drop_denum_ono"),0) * 100 ) AS "TCH_DROP_RATE",
      ...
      GROUP BY
        ...
        t1."BTS ID"
      HAVING
        (SUM(t1."TCH_Drop_num_ono") / NULLIF(SUM(t1."TCH_Drop_denum_ono"),0) * 100 ) > 1 
      ORDER BY
        (SUM(t1."TCH_Drop_num_ono") ) DESC
      LIMIT 20;
    `);
