  ...
  try {
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg2gModel>(sql`
          SELECT
            ...
            t2."KABUPATEN" as "KABUPATEN",
            CASE
              WHEN RIGHT (t1."BTS Name", 4) LIKE'%MD%'
                OR RIGHT (t1."BTS Name", 4) LIKE'%ID%'
                OR RIGHT (t1."BTS Name", 4) LIKE'%HD%'
                OR RIGHT (t1."BTS Name", 4) LIKE'%SD%'
                OR RIGHT (t1."BTS Name", 4) LIKE'%ED%' THEN
                'DCS'
              WHEN RIGHT (t1."BTS Name", 4) LIKE'%MG%'
                OR RIGHT (t1."BTS Name", 4) LIKE'%HG%'
                OR RIGHT (t1."BTS Name", 4) LIKE'%OG%'
                OR RIGHT (t1."BTS Name", 4) LIKE'%IG%' THEN
                'GSM'
              ELSE
                'Unknown'
            END AS "BAND",
          ...
          WHERE
            ...
            AND t2."KABUPATEN" like ${queryKabupaten}
          GROUP BY
            t1."Begin Time",
            t2."KABUPATEN",
            CASE
