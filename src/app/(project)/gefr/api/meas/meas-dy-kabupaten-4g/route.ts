  ...
  try {
    ...
    const result = await db_gefrdb_suldbv1.execute<Agg4gModel>(sql`
          SELECT
            ...
            t2."KABUPATEN",
            CASE
              WHEN "right" (t1."cellId" :: TEXT, 1) = '1' :: TEXT THEN
                'LTE 1800' :: TEXT
              WHEN "right" (t1."cellId" :: TEXT, 1) = '2' :: TEXT THEN
                'LTE 900' :: TEXT
              WHEN "right" (t1."cellId" :: TEXT, 1) = '3' :: TEXT THEN
                'LTE 2100' :: TEXT
              WHEN "right" (t1."cellId" :: TEXT, 1) = '4' :: TEXT THEN
                'LTE 2300' :: TEXT
              WHEN "right" (t1."cellId" :: TEXT, 1) = '6' :: TEXT THEN
                'LTE 2300' :: TEXT
              WHEN "right" (t1."cellId" :: TEXT, 1) = '5' :: TEXT THEN
                CASE
                  WHEN LENGTH(t1."cellId" :: TEXT) = 2 THEN
                    'LTE 2300' :: TEXT
          ...
          FROM
            ...
            END = t2."Site ID"
          WHERE
            t1."Begin Time" >= '9/20/2025' :: TIMESTAMP
            AND t1."Begin Time" <= '9/25/2025' :: TIMESTAMP
          GROUP BY
            t1."Begin Time",
            t2."KABUPATEN",
            t1."cellId"
