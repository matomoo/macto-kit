...
export async function GET(request: Request) {
  ...
  // console.log(queryNOP, queryKabupaten, queryBatch);

  try {
    const formattedTgl1 = new Date(tgl_1).toISOString();
    const formattedTgl2 = new Date(tgl_2).toISOString();

    // Build conditional batch filter
    const batchCondition = batch !== "All" ? sql`AND t2."Batch" LIKE ${queryBatch}` : sql``;

    const result = await db_gefrdb_suldbv1.execute<Agg2gModel>(
      sql`
        SELECT
